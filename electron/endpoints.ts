 ...
 * while working on SPYDIR issues a key against the local database and the "get a free key" link
 * lands somewhere that exists. SPYDIR_LICENSE_API overrides both, for pointing a build at staging.
 *
 ...
 * shipped SPYDIR has no reason to talk to anywhere but production.
 */
...
export function siteBase(): string {
  ...
  const override = process.env.SPYDIR_LICENSE_API?.trim()
