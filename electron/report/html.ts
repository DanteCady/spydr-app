...
<head>
<meta charset="utf-8" />
<title>SPYDR — ${escapeHtml(report.title)}</title>
<style>
${fontCss}
${STYLE}
</style>
...
const MARK = '<svg viewBox="0 0 1114 1280" fill="currentColor"><use href="#spydr-mark"/></svg>';
const WORD = '<span class="word">SPY<em>DR</em></span>';
const host = document.getElementById('sheets');
document.body.insertAdjacentHTML('afterbegin',
  '<svg width="0" height="0" style="position:absolute" aria-hidden="true"><symbol id="spydr-mark" viewBox="0 0 1114 1280">' +
...
function layout() {
  ...
  sheets.forEach((s, i) => {
    ...
    spans[0].textContent = 'SPYDR · ' + REPORT.meta.domain + ' · read-only directory report';
